...
void CPUMatrix<ElemType>::BatchMatMul(const CPUMatrix<ElemType>& a, const bool transposeA, const int m, const CPUMatrix<ElemType>& b, const bool transposeB, const int n, CPUMatrix<ElemType>& c, const bool isColWise)
{
    ...
    for (size_t i = 0; i < aBatchSize; i++)
    {
        if (sizeof(ElemType) == sizeof(double))
        {
            double alpha = 1.0;
            double beta = 0.0;
            cblas_dgemm((CBLAS_ORDER)(int)MatrixOrder::ColMajor, blasTransA, blasTransB, m, n, k, alpha, reinterpret_cast<const double*>(a_array[i]), lda, reinterpret_cast<const double*>(b_array[i]), ldb, beta, reinterpret_cast<double*>(c_array[i]), ldc);
        }
        else
        {
            float alpha = 1.0;
            float beta = 0.0;
            cblas_sgemm((CBLAS_ORDER)(int)MatrixOrder::ColMajor, blasTransA, blasTransB, m, n, k, alpha, reinterpret_cast<const float*>(a_array[i]), lda, reinterpret_cast<const float*>(b_array[i]), ldb, beta, reinterpret_cast<float*>(c_array[i]), ldc);
        }
    }
